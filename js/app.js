...
const App = {
    ...
    init() {
        this.levels = [LEVEL1_DATA, LEVEL2_DATA, LEVEL3_DATA, LEVEL4_DATA, LEVEL5_DATA];
        this.allModules = [];
        ...
        this.buildSidebar();
        this.showDashboard();
        if (localStorage.getItem('mgmt-accounting-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('mgmt-accounting-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('mgmt-accounting-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('mgmt-accounting-progress', JSON.stringify(this.progress));
        localStorage.setItem('mgmt-accounting-quiz-results', JSON.stringify(this.quizResults));
    },
    ...
    buildSidebar() {
        ...
        this.levels.forEach(level => {
            ...
            levelModules.forEach(mod => {
                ...
                html += `
                    <div class="sidebar-item ${isCompleted ? 'completed' : ''} ${isActive ? 'active' : ''}" onclick="App.showModule(${mod.id})">
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>管理会計アカデミーへようこそ</h2>
                    <p>経営の意思決定を数字で武装する。管理会計を基礎から実践まで学べる総合教育プラットフォームです。</p>
                </div>
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = `
            ...
            <p>
                全モジュールを完了しました！<br>
                管理会計の基礎から実践まで一通り学びました。<br>
                数字で語り、数字で決め、数字で検証する経営を実践しましょう！
            </p>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('mgmt-accounting-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('mgmt-accounting-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('mgmt-accounting-progress');
            localStorage.removeItem('mgmt-accounting-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
